<template>
  ...
  >
    ...
    <el-form-item label="规格" prop="specification">
      <goods-spec-select
        ref="goods-spec-select"
        :type="ruleForm.specType"
    ...
    <el-form-item label="状态" prop="commodityStatus">
      <el-switch
        v-model="ruleForm.commodityStatus"
        active-text="上架"
        inactive-text="下架"
        :active-value="1"
        :inactive-value="0"
    ...
    <el-form-item
      label="是否允许老学员购买"
      v-if="ruleForm.deliveryMethod == 0"
    >
    ...
    <el-form-item v-if="ruleForm.userType == 1" prop="userArgs">
      <el-transfer
        v-loading="loading"
        filterable
        filter-placeholder="请输入用户手机号"
        :titles="['用户列表', '已选用户']"
        v-model="ruleForm.userArgs"
        :data="userList"
      >
        <span slot-scope="{ option }"
          >{{ option.label }} - {{ option.nickName }}</span
        >
      </el-transfer>
    </el-form-item>
    <div v-if="ruleForm.userType == 2">
      <el-form-item style="width: 100%" label="实名状态">
        <el-checkbox
          :indeterminate="useCcertificationIndeterminate"
          v-model="useCcertificationCheckAll"
          @change="useCcertificationCheckAllChange"
          >全选</el-checkbox
        >
        <el-checkbox-group
          v-model="userArgs.userCertification"
          @change="useCcertificationCheckedCitiesChange"
        >
          <el-checkbox
            v-for="item in useCcertificationDict"
            :label="item.value"
            :key="item.value"
          >
      ...
      <el-form-item style="width: 100%" label="球迷会">
        <el-checkbox
          :indeterminate="fanClubIdIndeterminate"
          v-model="fanClubIdCheckAll"
          @change="fanClubIdCheckAllChange"
          >全选</el-checkbox
        >
        <el-checkbox-group
          v-model="userArgs.fanClubId"
          @change="fanClubIdCheckedCitiesChange"
        >
          <el-checkbox
            v-for="item in fanClubList"
            :label="item.value"
            :key="item.value"
          >
            {{ item.text }}
      ...
      <el-form-item style="width: 100%" label="套票类型">
        <el-checkbox
          :indeterminate="ticketPackageUserIndeterminate"
          v-model="ticketPackageUserCheckAll"
          @change="ticketPackageUserCheckAllChange"
          >全选</el-checkbox
        >
        <el-checkbox-group
          v-model="userArgs.ticketPackageUser"
          @change="ticketPackageUserCheckedCitiesChange"
        >
          <el-checkbox
            v-for="item in ticketPackageUserDict"
            :label="item.value"
            :key="item.value"
          >
            {{ item.text }}</el-checkbox
      ...
      <el-form-item style="width: 100%" label="会员等级">
        <el-checkbox
          :indeterminate="vipLevelIndeterminate"
          v-model="vipLevelCheckAll"
          @change="vipLevelCheckAllChange"
          >全选</el-checkbox
        >
        <el-checkbox-group
          v-model="userArgs.vipLevel"
          @change="vipLevelCheckedCitiesChange"
        >
          <el-checkbox
            v-for="item in vipLevelDict"
            :label="item.value"
            :key="item.value"
          >
            {{ item.text }}
      ...
      <el-form-item style="width: 100%" label="支云卡状态">
        <el-checkbox
          :indeterminate="zhiyunCardStatusIndeterminate"
          v-model="zhiyunCardStatusCheckAll"
          @change="zhiyunCardStatusCheckAllChange"
          >全选</el-checkbox
        >
        <el-checkbox-group
          v-model="userArgs.zhiyunCardStatus"
          @change="zhiyunCardStatusCheckedCitiesChange"
        >
          <el-checkbox
            v-for="item in zhiyunCardStatusDict"
            :label="item.value"
            :key="item.value"
          >
            {{ item.text }}
      ...
      <el-form-item style="width: 100%" label="青训用户">
        <el-checkbox
          :indeterminate="trainingStatusIndeterminate"
          v-model="trainingStatusCheckAll"
          @change="trainingStatusCheckAllChange"
          >全选</el-checkbox
        >
        <el-checkbox-group
          v-model="userArgs.trainingStatus"
          @change="trainingStatusCheckedCitiesChange"
        >
          <el-checkbox
            v-for="item in trainingStatusDict"
            :label="item.value"
            :key="item.value"
          >
            {{ item.text }}
    ...
    <el-form-item label="填写信息">
      <el-checkbox-group
        ...
        v-model="ruleForm.infoField"
        size="small"
        @change="infoFieldChange"
      >
        <el-checkbox
          v-for="(item, index) in infoFieldList"
          :key="index"
          :label="item.value"
          border
      ...
      <div v-else class="flex flex-wrap">
        <div
          v-for="item in infoFieldList"
          :key="item.value"
          class="editing-items"
        >
          <span class="mr-2">{{ item.label }}</span>
          <template v-if="!requireField.includes(item.value)">
            <el-button size="small" type="text" @click="editField(item)"
              >编辑</el-button
            >
            <el-button size="small" type="text" @click="removeField(item)"
              >删除</el-button
            >
          </template>
        </div>
      ...
      <div v-if="!!editingInfoField">
        <el-button size="small" type="primary" @click="editingInfoField = null"
          >结束编辑
        </el-button>
      </div>
      <div v-else>
        <el-button size="small" type="primary" @click="infoFieldDialog = true"
          >添加字段
        </el-button>
        <el-button size="small" type="primary" @click="editingInfoField = true"
          >编辑字段
        </el-button>
      </div>
    </el-form-item>
    ...
    <el-form-item
      label="协议文件"
      v-if="ruleForm.infoField.some((el) => el == 'pact')"
      prop="pact"
    >
      <div v-for="(item, i) in ruleForm.pact" :key="i" class="flex">
        ...
        ></cl-upload>
        <div class="ml-2">
          <el-button size="small" @click="ruleForm.pact.splice(i, 1)"
            >删除</el-button
          >
        </div>
      </div>
    ...
    <el-dialog
      width="30%"
      title="添加字段"
      :visible.sync="infoFieldDialog"
      append-to-body
      @close="resetInfoField('infoForm')"
    >
      <el-form
        :model="infoForm"
        :inline="false"
      ...
      >
        ...
        <el-form-item label="采集形式" required>
          <el-radio-group v-model="infoForm.formType">
            <el-radio :disabled="!!editingInfoField" label="text"
              >文本输入</el-radio
            >
            <el-radio :disabled="!!editingInfoField" label="radio"
              >单选</el-radio
            >
            <el-radio :disabled="!!editingInfoField" label="checkbox"
              >多选</el-radio
            >
            <el-radio :disabled="!!editingInfoField" label="image" class="mt-2"
              >上传图片</el-radio
            >
            <el-radio :disabled="!!editingInfoField" label="video" class="mt-2"
              >上传视频</el-radio
            >
          </el-radio-group>
        </el-form-item>
        <el-form-item
          prop="selectList"
          label="选项"
          v-if="['radio', 'checkbox'].includes(infoForm.formType)"
          required
        >
        ...
        <el-form-item>
          <el-button
            size="small"
            type="primary"
            @click="addInfoField('infoForm')"
            >确定</el-button
          >
          <el-button
            size="small"
            v-show="!editingInfoField"
            @click="resetInfoField('infoForm')"
            >重置</el-button
          >
        </el-form-item>
      </el-form>
...
export default {
        ...
        value: [
          ...
          },
          {
            validator(rules, value, callback) {
              if (
                !vm.editingInfoField &&
                vm.infoFieldList.find((el) => el.value == value)
              ) {
  ...
  async created() {
    await this.$store.dispatch(
      "setDefaultcolumn",
      this.item?.goodsType == 2 ? specColumnScore : specColumn
    );
  ...
  methods: {
    ...
    useCcertificationCheckAllChange(val) {
      this.userArgs.userCertification = val
        ? this.useCcertificationCheckbox
        : [];
      this.useCcertificationIndeterminate = false;
    },
    useCcertificationCheckedCitiesChange(value) {
      let checkedCount = value.length;
      this.useCcertificationCheckAll =
        checkedCount == this.useCcertificationCheckbox.length;
      this.useCcertificationIndeterminate =
        checkedCount > 0 &&
        checkedCount < this.useCcertificationCheckbox.length;
    },
    ...
    fanClubIdCheckedCitiesChange(value) {
      ...
      this.fanClubIdIndeterminate =
        checkedCount > 0 && checkedCount < this.fanClubIdCheckbox.length;
    },
    ticketPackageUserCheckAllChange(val) {
      this.userArgs.ticketPackageUser = val
        ? this.ticketPackageUserCheckbox
        : [];
      this.ticketPackageUserIndeterminate = false;
    },
    ticketPackageUserCheckedCitiesChange(value) {
      let checkedCount = value.length;
      this.ticketPackageUserCheckAll =
        checkedCount === this.ticketPackageUserCheckbox.length;
      this.ticketPackageUserIndeterminate =
        checkedCount > 0 &&
        checkedCount < this.ticketPackageUserCheckbox.length;
    },
    ...
    vipLevelCheckedCitiesChange(value) {
      ...
      this.vipLevelIndeterminate =
        checkedCount > 0 && checkedCount < this.vipLevelCheckbox.length;
    },
    ...
    zhiyunCardStatusCheckedCitiesChange(value) {
      let checkedCount = value.length;
      this.zhiyunCardStatusCheckAll =
        checkedCount === this.zhiyunCardStatusCheckbox.length;
      this.zhiyunCardStatusIndeterminate =
        checkedCount > 0 && checkedCount < this.zhiyunCardStatusCheckbox.length;
    },
    ...
    trainingStatusCheckedCitiesChange(value) {
      let checkedCount = value.length;
      this.trainingStatusCheckAll =
        checkedCount === this.trainingStatusCheckbox.length;
      this.trainingStatusIndeterminate =
        checkedCount > 0 && checkedCount < this.trainingStatusCheckbox.length;
    },
    removeField(item) {
