	...
	<div class="network">
		...
		<ul>
			<li
				class="rx"
				@click="change('上行')"
				:class="[options.legend.selected['上行'] ? 'on' : 'off']"
			>
				<p>上行</p>
			...
			<li
				class="tx"
				@click="change('下载')"
				:class="[options.legend.selected['下载'] ? 'on' : 'off']"
			>
				<p>下载</p>
		...
		<e-charts v-if="visible" :options="options"></e-charts>
	</div>
</template>
...
export default {
	...
	methods: {
		change(n) {
			this.options.legend.selected[n] = !this.options.legend.selected[n];
			this.visible = false;
		...
		refresh(res) {
			...
			this.options.series[0].name = "上行";
			this.options.series[0].data = data.map((e) =>
				e.server.network.reduce((a, b) => a + b.tx_sec, 0)
			);

			this.options.series[1].name = "下载";
			this.options.series[1].data = data.map((e) =>
